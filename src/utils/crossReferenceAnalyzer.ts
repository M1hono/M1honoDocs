...
export interface CrossReference {
  ...
  referenceType: 'import' | 'extends' | 'implements' | 'field' | 'method_call' | 'parameter' | 'return_type' | 'annotation';
  /** 引用的具体位置 */
  location: {
    line: number;
...
export class CrossReferenceAnalyzer {
  ...
  async analyzeClassUsage(className: string): Promise<ClassUsage> {
    ...
    for (const [, classDoc] of this.docIndex.classes) {
      const classRefs = await this.findClassReferencesInClass(className, classDoc);
    ...
    // 计算被多少个不同的类引用
    const referencedBySet = new Set(references.map(ref => ref.referencedBy));
   ...
   * 分析指定方法的使用情况
   */
  async analyzeMethodUsage(className: string, methodName: string): Promise<MethodUsage> {
    const calls: CrossReference[] = [];
    
    // 遍历所有类，查找对指定方法的调用
    for (const [, classDoc] of this.docIndex.classes) {
      const methodCalls = await this.findMethodCallsInClass(className, methodName, classDoc);
   ...
   * 在指定类中查找对目标类的引用
   */
  private async findClassReferencesInClass(targetClass: string, classDoc: JavaClassDoc): Promise<CrossReference[]> {
    const references: CrossReference[] = [];
    ...
    if (classDoc.superClass === targetClass) {
      const line = this.findLineContaining(lines, `extends ${targetClass}`);
    ...
    if (classDoc.interfaces.includes(targetClass)) {
      const line = this.findLineContaining(lines, `implements.*${targetClass}`);
      if (line.lineNumber !== -1) {
        references.push({
          referencedBy: classDoc.fullName,
          filePath: classDoc.filePath,
    ...
    // 检查 import 语句
    const importLine = this.findLineContaining(lines, `import.*${targetClass}`);
    ...
    // 检查方法体中的使用（简单的文本匹配）
    const methodBodyRefs = this.findMethodBodyReferences(lines, targetClass, classDoc);
   ...
   * 在指定类中查找对目标方法的调用
   */
  private async findMethodCallsInClass(targetClass: string, methodName: string, classDoc: JavaClassDoc): Promise<CrossReference[]> {
    const calls: CrossReference[] = [];
    const sourceCode = await this.getSourceCode(classDoc.filePath);
    const lines = sourceCode.split('\n');
   ...
   * 在方法体中查找类的引用
   */
  private findMethodBodyReferences(lines: string[], targetClass: string, classDoc: JavaClassDoc): CrossReference[] {
    const references: CrossReference[] = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const staticCallPattern = new RegExp(`${targetClass}\\.\\w+\\s*\\(`, 'g');
   ...
   * 查找包含指定内容的行
   */
  private findLineContaining(lines: string[], pattern: string): { lineNumber: number; content: string } {
   ...
   * 获取热门类（被引用最多的类）
   */
  async getPopularClasses(limit: number = 10): Promise<{ className: string; usage: ClassUsage }[]> {
    ...
    // 按引用次数排序
    classUsages.sort((a, b) => b.usage.totalReferences - a.usage.totalReferences);
