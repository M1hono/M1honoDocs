...
export class DocScanner {
   ...
   * 递归扫描目录
   */
  private async scanDirectory(nodes: FileNode[], allClasses: JavaClassDoc[]): Promise<void> {
   ...
   * 搜索类、方法、字段
   */
  search(docIndex: ProjectDocIndex, query: string): {
    ...
    for (const classDoc of docIndex.classes.values()) {
      ...
      if (
        ...
        (classDoc.classComment && classDoc.classComment.toLowerCase().includes(lowerQuery))
      ) {
      ...
      for (const method of classDoc.methods) {
        if (
          ...
          (method.comment && method.comment.toLowerCase().includes(lowerQuery)) ||
      ...
      for (const field of classDoc.fields) {
        if (
          ...
          (field.comment && field.comment.toLowerCase().includes(lowerQuery))
        ) {
