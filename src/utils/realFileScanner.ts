...
export class RealFileScanner {
    ...
    async scanAllJavaFiles(): Promise<FileNode[]> {
        ...
        // 定义要扫描的根路径
        const rootPaths = ["/forge-1.20.1-47.1.99", "/KubeJS-2001"];
    ...
    private async validateJavaFile(filePath: string): Promise<boolean> {
        try {
            ...
            const isReasonableSize =
                content.length > 10 && content.length < 1000000;
    ...
    getStats() {
        return {
            ...
            javaFiles: this.foundFiles.filter((f) => f.name.endsWith(".java"))
                .length,
        };
    }
}
