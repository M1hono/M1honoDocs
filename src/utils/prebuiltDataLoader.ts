...
export class PrebuiltDataLoader {
     ...
     * 按需加载包的类数据
     */
    async loadPackageClasses(
        packageName: string
    ): Promise<Map<string, JavaClassDoc>> {
    ...
    async loadClassDoc(className: string): Promise<JavaClassDoc | null> {
        ...
        if (parts.length < 2) {
            console.error(
                `Invalid class name (not enough parts): ${className}`
            );
            return null;
        }
        ...
        console.log(
            `Determined package [${packageName}] for class [${className}]`
        );
        ...
        console.log(
            `Direct match failed. Trying to find as inner class via parent: ${topLevelClassFullName}`
        );

        const parentClassDoc = packageClasses.get(topLevelClassFullName);
        if (parentClassDoc) {
            const innerClass = this.searchNestedInnerClasses(
                parentClassDoc,
                className
            );
            if (innerClass) {
     ...
     * 递归搜索嵌套的内部类
     */
    private searchNestedInnerClasses(
        ...
        for (const innerClass of classDoc.innerClasses) {
            ...
            // 递归搜索
            const nestedResult = this.searchNestedInnerClasses(
                innerClass,
                targetClassName
            );
            if (nestedResult) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (text.startsWith("<!DOCTYPE html>")) {
                    console.error(
                        `Failed to load source for ${filePath}, server returned HTML.`
                    );
                ...
                console.error(
                    `Failed to load source for ${filePath}, status: ${response.status}`
                );
    ...
    > {
        ...
        try {
            ...
            for (const [className, hierarchy] of Object.entries(
                hierarchyData
            )) {
    ...
    > {
        ...
        for (const [packageName, classList] of docIndex.packages) {
            if (packageName.toLowerCase().includes(queryLower)) {
                // 如果包名匹配，加载包中的所有类
                const packageClasses = await this.loadPackageClasses(
                    packageName
                );
    ...
    private async loadJSON(url: string): Promise<any> {
        try {
            ...
            if (text.startsWith("<!DOCTYPE html>")) {
                console.error(
                    `Received HTML page instead of JSON data from: ${url}`
                );
                throw new Error(
                    `Received HTML page instead of JSON data: ${url}`
                );
            }
